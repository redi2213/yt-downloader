...
def pick_format(formats, target_height, want_hdr):
    # formats: list of (fmt_id, label) e.g. ("337", "2160p60 HDR")
    candidates = []
...
def process_link(url, target_height, want_hdr):
    print(f"\n===== Processing: {url} =====")
    ...
    if conclusion != "success":
        print(f"[FAIL] could not list formats for {url}")
        return
    log_text = get_run_log_text(run_id)
    formats = parse_formats(log_text)
    if not formats:
        print(f"[FAIL] no formats found for {url}")
        return

    picked = pick_format(formats, target_height, want_hdr)
    if not picked:
        print(f"[FAIL] could not pick a format for {url}")
        return
    fmt_id, label = picked
    print(f"[PICK] {label} (id={fmt_id})")
